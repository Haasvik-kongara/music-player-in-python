...
def load_music():
    ...
    cureent_song = songs[songlist.curselection()[0]]


def playmusic():
    global cureent_song, paused
    if not paused:
        pygame.mixer.music.load(os.path.join(root.directory,cureent_song))
...
def prev_music():
    ...
    try:
        songlist.selection_clear(0,END)
        songlist.selection_set(songs.index(cureent_song)-1) 
        cureent_song=songs[songlist.curseselection()[0]]
        playmusic()
    except:
        pass 
